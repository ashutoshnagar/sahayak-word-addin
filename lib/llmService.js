    ...
    for (const issue of analysisResult.issues) {
      try {
        ...
        if (actualText !== issue.location.exactText) {
          ...
          if (this.fuzzyMatch(actualText, issue.location.exactText, 0.3)) { // More lenient threshold
            issue.location.exactText = actualText; // Update to actual text
            console.log(`Fuzzy match accepted for issue ${issue.id}`);
          } else {
            // For POC, be more lenient - just update the text and continue
            validationStats.textMismatches++;
            console.warn(`Text mismatch for issue ${issue.id} - accepting anyway for POC:`);
            console.warn(`Expected: "${issue.location.exactText}"`);
            console.warn(`Actual: "${actualText}"`);
            issue.location.exactText = actualText; // Use actual text
            // Don't skip the issue - continue processing
          }
        }
